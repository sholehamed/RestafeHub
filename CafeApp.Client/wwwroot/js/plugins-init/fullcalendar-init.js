! function(e) {
    ...
    t.prototype.onDrop = function(t, n) {
        ...
        var o = this,
            i = e("<form></form>");
        i.append("<label>تغییر نام ایونت</label>"), i.append("<div class='input-group'><input class='form-control' type=text value='" + t.title + "' /><span class='input-group-btn'><button type='submit' class='btn btn-success waves-effect waves-light'><i class='fa fa-check'></i> ذخیره</button></span></div>"), o.$modal.modal({
            backdrop: "static"
        ...
        var i = e("<form></form>");
        i.append("<div class='row'></div>"), i.find(".row").append("<div class='col-md-6'><div class='form-group'><label class='control-label'>ایونت جدید</label><input class='form-control' placeholder='لطفا اسم ایونت رو بنویس' type='text' name='title'/></div></div>").append("<div class='col-md-6'><div class='form-group'><label class='control-label'>دسته بندی</label><select class='form-control' name='category'></select></div></div>").find("select[name='category']").append("<option value='bg-danger'>خطر</option>").append("<option value='bg-success'>موفقیت</option>").append("<option value='bg-dark'>تیره</option>").append("<option value='bg-primary'>اصلی</option>").append("<option value='bg-pink'>ثانویه </option>").append("<option value='bg-info'>اطلاعات</option>").append("<option value='bg-warning'>هشدار</option></div></div>"), o.$modal.find(".delete-event").hide().end().find(".save-event").show().end().find(".modal-body").empty().prepend(i).end().find(".save-event").unbind("click").on("click", function() {
            i.submit()
        }), o.$modal.find("form").on("submit", function() {
            ...
            return null !== e && 0 != e.length ? (o.$calendarObj.fullCalendar("renderEvent", {
                ...
                allDay: !1,
                className: a
            }, !0), o.$modal.modal("hide")) : alert("باید یه اسم برای ایونتت انتخاب کنی"), !1
        ...
        var t = new Date,
            ...
            a = [{
                ...
                title: "بررسی فاکتورها",
                start: n,
                end: n,
                className: "bg-danger"
            }, {
                title: "تمدید قرارداد با پرسنل",
        ...
        o.$calendarObj = o.$calendar.fullCalendar({
            ...
            null !== e && 0 != e.length && (o.$extEvents.append('<div class="external-event btn-' + t + ' light" data-class="bg-' + t + '" style="position: relative;"><i class="fa fa-move"></i>' + e + "</div>"), o.enableDrag())
        })
